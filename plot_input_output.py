...
def plot_input_output(title,
    ...
    ax_x_inv = fig.add_subplot(gs[:, 1], sharex=ax_x)
    ax_diff = fig.add_subplot(gs[:, 2], sharex=ax_x)
    ax_y = fig.add_subplot(gs[:, 3], sharex=ax_x)
    ...
    ax.text(length * 0.3, 5, 'pitch activation', fontdict=dict(size=font_size))
